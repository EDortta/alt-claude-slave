...
exec ssh -t "$DOM1_SSH_TARGET" \
  "sudo -n incus exec '$CONTAINER_NAME' -- bash -lc '
    while true; do
      ...
      echo
      echo "--- processos ---"
      ps -eo pid,etime,%cpu,%mem,rss,cmd --sort=-%cpu | grep -E "[s]lave_worker.py|[l]lama-(completion|cli|server)" | head -n 12 || echo "nenhum processo ativo"
      echo
