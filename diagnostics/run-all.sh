...
say(){ printf '[diagnóstico] %s\n' "$*"; }

run_shell(){
  local name="$1" command="$2" rc=0
  CURRENT_STEP="$name"; say "$name ..."
  { echo "+ $command"; bash -lc "$command" || rc=$?; echo; echo "exit=$rc"; } >"$REPORT_DIR/$name.txt" 2>&1
  if (( rc == 0 )); then say "$name: OK"; else say "$name: FALHOU (exit=$rc) — registrado em $name.txt"; fi
  return 0
}
...
run_shell 07-container "ssh -T -o BatchMode=yes -o ConnectTimeout=10 '$DOM1_SSH_TARGET' 'sudo -n incus exec \"$CONTAINER_NAME\" -- bash -lc '\''id; uname -a; python3 --version; git --version; echo; ls -lah \"$REMOTE_REPO\"; echo; runuser -u slave -- git -C \"$REMOTE_REPO\" status --short --branch; runuser -u slave -- git -C \"$REMOTE_REPO\" log -3 --oneline --decorate; echo; ls -lah \"$REMOTE_REPO/scripts/slave_mcp.py\" \"$REMOTE_REPO/slave-mcp\" 2>&1 || true'\'''"
run_shell 08-container-tests "ssh -T -o BatchMode=yes -o ConnectTimeout=10 '$DOM1_SSH_TARGET' 'sudo -n incus exec \"$CONTAINER_NAME\" -- runuser -u slave -- bash -lc '\''cd \"$REMOTE_REPO\" && bash tests/run.sh'\'''"
...
CURRENT_STEP="09-bridge-initialize"; say "09-bridge-initialize ..."; rc=0
{ echo "+ initialize via local bridge"; if [[ -x "$BRIDGE" ]]; then printf '%s\n' "$INIT" | timeout 25 "$BRIDGE" || rc=$?; else echo bridge_not_executable; rc=127; fi; echo; echo "exit=$rc"; } >"$REPORT_DIR/09-bridge-initialize.txt" 2>&1
(( rc == 0 )) && say "09-bridge-initialize: OK" || say "09-bridge-initialize: FALHOU (exit=$rc)"

CURRENT_STEP="10-bridge-tools-list"; say "10-bridge-tools-list ..."; rc=0
{ echo "+ initialize + tools/list via local bridge"; if [[ -x "$BRIDGE" ]]; then { printf '%s\n' "$INIT"; printf '%s\n' "$TOOLS"; } | timeout 25 "$BRIDGE" || rc=$?; else echo bridge_not_executable; rc=127; fi; echo; echo "exit=$rc"; } >"$REPORT_DIR/10-bridge-tools-list.txt" 2>&1
(( rc == 0 )) && say "10-bridge-tools-list: OK" || say "10-bridge-tools-list: FALHOU (exit=$rc)"
...
Para o teste funcional completo do worker, execute também:
  bash diagnostics/verify-mcp-worker.sh
EOF

CURRENT_STEP="done"
printf '\n[diagnóstico] CONCLUÍDO\n'
printf '[diagnóstico] relatório: %s\n' "$REPORT_DIR"
printf '[diagnóstico] próximo teste funcional:\n'
printf '  bash diagnostics/verify-mcp-worker.sh\n'
